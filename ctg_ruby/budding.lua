...
core.register_abm({
    label = "ruby cluster growth",
    nodenames = "ctg_ruby:ruby_budding",
    interval = 220,
    chance = 1,
    action = function(pos)
        ...
        if check_node_name == "air" then
            new_node = "ctg_ruby:cluster_small"
        else if check_node_name == "ctg_ruby:cluster_small" then
            new_node = "ctg_ruby:cluster_medium"
        else if check_node_name == "ctg_ruby:cluster_medium" then
            new_node = "ctg_ruby:cluster_large"
        else return end
        end
        end
